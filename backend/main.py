...
def _check_not_in_maintenance(db: Session, current_user: models.User):
    """Admins can keep using the app during maintenance mode (to verify a
    fix, etc.) — everyone else is blocked from generating new content."""
    settings = _get_settings(db)
    if settings.maintenance_mode and current_user.role != "admin":
        raise HTTPException(status_code=503, detail="Question generation is temporarily disabled for maintenance. Please try again shortly.")
...
def register(request: Request, req: RegisterRequest, db: Session = Depends(get_db)):
    ...
    settings = _get_settings(db)
    if not settings.allow_self_signup and not is_first_user:
        raise HTTPException(status_code=403, detail="Self sign-up is currently disabled. Ask your administrator for an invite.")
...
def generate(request: Request, req: GenerateRequest, db: Session = Depends(get_db),
             current_user: models.User = Depends(auth.get_current_user)):
    _check_not_in_maintenance(db, current_user)
...
def admin_create_announcement(req: CreateAnnouncementRequest, db: Session = Depends(get_db),
                               admin_user: models.User = Depends(auth.get_admin_user)):
    """Owner-only: broadcast a message to every user — it shows up in
    everyone's notification bell (merged into GET /activity)."""
    if not req.message.strip():
        raise HTTPException(status_code=400, detail="Announcement message can't be empty")
    ann = models.Announcement(message=req.message.strip(), created_by_id=admin_user.id)
    db.add(ann)
    db.commit()
    db.refresh(ann)
    return {"id": ann.id, "message": ann.message, "created_at": ann.created_at}


@app.get("/admin/announcements")
def admin_list_announcements(db: Session = Depends(get_db),
                              admin_user: models.User = Depends(auth.get_admin_user)):
    items = db.query(models.Announcement).order_by(models.Announcement.created_at.desc()).limit(20).all()
    return [{"id": a.id, "message": a.message, "created_at": a.created_at} for a in items]
...
def get_activity(db: Session = Depends(get_db),
    ...
    for a in recent_announcements:
        items.append({"id": f"announcement-{a.id}", "type": "announcement",
                       "text": a.message, "timestamp": a.created_at})
...
def admin_list_feedback(db: Session = Depends(get_db),
    ...
    for f in items:
        ...
        result.append({
            "id": f.id, "category": f.category, "message": f.message, "created_at": f.created_at,
            "reply": f.reply, "reply_at": f.reply_at, "status": f.status or "open",
...
def admin_update_feedback_status(feedback_id: int, req: UpdateFeedbackStatusRequest, db: Session = Depends(get_db),
                                  admin_user: models.User = Depends(auth.get_admin_user)):
    """Owner-only: mark a ticket resolved/reopened, independent of replying."""
...
def admin_list_users(db: Session = Depends(get_db),
    ...
    result = []
    for u in users:
        paper_count = db.query(models.Paper).filter(models.Paper.owner_id == u.id).count()
        result.append({
            "id": u.id, "name": u.name, "email": u.email, "role": u.role,
            "is_verified": u.is_verified, "is_suspended": u.is_suspended, "papers": paper_count,
        })
    return result
...
def admin_toggle_user_suspension(user_id: int, req: SuspendUserRequest, db: Session = Depends(get_db),
                                  admin_user: models.User = Depends(auth.get_admin_user)):
    """Owner-only: suspend or reinstate a user. Suspension is enforced
    immediately — a suspended user is rejected on their very next request,
    not just at their next login (see get_current_user)."""
    user = db.query(models.User).filter(models.User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    if user.role == "admin":
        raise HTTPException(status_code=400, detail="Can't suspend an admin/owner account")

    user.is_suspended = req.is_suspended
    db.commit()
    db.refresh(user)
...
def admin_list_papers(db: Session = Depends(get_db),
                       admin_user: models.User = Depends(auth.get_admin_user)):
    """Owner-only: every paper built on the platform, across every teacher —
    for moderation and support, not just the current user's own papers."""
    ...
    for p in papers:
        ...
        question_ids = [i for i in p.question_ids.split(",") if i]
        marks_total = 0
        if question_ids:
            questions = db.query(models.Question).filter(models.Question.id.in_([int(i) for i in question_ids])).all()
            marks_total = sum(q.marks or 0 for q in questions)
        result.append({
            "id": p.id, "title": p.paper_name, "author": owner.name if owner else "Unknown",
            "questions": len(question_ids), "marks": marks_total,
            "created_at": p.created_at, "file_type": p.file_type,
        })
    return result
...
def admin_delete_paper(paper_id: int, db: Session = Depends(get_db),
                        admin_user: models.User = Depends(auth.get_admin_user)):
    """Owner-only: remove any paper from the platform (moderation)."""
    paper = db.query(models.Paper).filter(models.Paper.id == paper_id).first()
    if not paper:
        raise HTTPException(status_code=404, detail="Paper not found")

    file_path = os.path.join(GENERATED_DIR, f"paper_{paper.id}.{paper.file_type}")
...
class UpdateSettingsRequest(BaseModel):
    ...
    bilingual_enabled: Optional[bool] = None
...
def admin_get_settings(db: Session = Depends(get_db),
    ...
    s = _get_settings(db)
    return {
        "maintenance_mode": s.maintenance_mode, "allow_self_signup": s.allow_self_signup,
        "bilingual_enabled": s.bilingual_enabled, "daily_rate_limit": s.daily_rate_limit,
    }


@app.patch("/admin/settings")
def admin_update_settings(req: UpdateSettingsRequest, db: Session = Depends(get_db),
                           admin_user: models.User = Depends(auth.get_admin_user)):
    """Owner-only: these are enforced for real, not just stored — maintenance_mode
    blocks question generation (see _check_not_in_maintenance) and
    allow_self_signup gates new registrations (see /auth/register)."""
    s = _get_settings(db)
    if req.maintenance_mode is not None:
        s.maintenance_mode = req.maintenance_mode
    if req.allow_self_signup is not None:
        s.allow_self_signup = req.allow_self_signup
    if req.bilingual_enabled is not None:
        s.bilingual_enabled = req.bilingual_enabled
    if req.daily_rate_limit is not None:
        s.daily_rate_limit = req.daily_rate_limit
    db.commit()
    db.refresh(s)
    return {
        "maintenance_mode": s.maintenance_mode, "allow_self_signup": s.allow_self_signup,
...
def admin_overview(db: Session = Depends(get_db),
                    admin_user: models.User = Depends(auth.get_admin_user)):
    """Owner-only: platform-wide stats for the admin dashboard's Overview tab.
    Every number here is computed from real data — there's no uptime/AI-quota
    tracking in this app, so 'AI calls today' is honestly labelled as
    questions generated today rather than pretending to measure something
    we don't actually instrument."""
    total_users = db.query(models.User).count()
    total_papers = db.query(models.Paper).count()
    active_teachers = db.query(models.User).filter(
        models.User.role == "teacher", models.User.is_suspended == False  # noqa: E712
    ).count()

    today = datetime.utcnow().date()
    questions_today = db.query(models.Question).filter(
        func.date(models.Question.created_at) == today
    ).count()
    ...
    for i in range(7):
        ...
        count = db.query(models.Paper).filter(func.date(models.Paper.created_at) == day).count()
        papers_trend.append({"day": day.strftime("%a"), "papers": count})

    bloom_rows = db.query(models.Question.bloom_level, func.count(models.Question.id)).group_by(models.Question.bloom_level).all()
    bloom_counts = {level: count for level, count in bloom_rows}
    total_questions = sum(bloom_counts.values()) or 1
    bloom_coverage = [
        {"level": level, "pct": round(100 * bloom_counts.get(level, 0) / total_questions)}
        for level in ["Remember", "Understand", "Apply", "Analyze", "Evaluate", "Create"]
    ]

    recent_papers = db.query(models.Paper).order_by(models.Paper.created_at.desc()).limit(5).all()
    activity = []
    for p in recent_papers:
        owner = db.query(models.User).filter(models.User.id == p.owner_id).first()
        activity.append({
            "who": owner.name if owner else "Unknown", "what": "built a paper",
            "where": p.paper_name, "when": p.created_at,
        })
