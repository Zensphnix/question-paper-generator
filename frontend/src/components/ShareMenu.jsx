...
export default function ShareMenu({ paperId, paperName, fileType }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 hover:text-violet-600 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-1.5 transition"
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 mt-2 w-72 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl shadow-xl z-50 p-4 space-y-3"
          >
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium text-slate-900 dark:text-white">{t("sharePaper")}</p>
            ...
            <div className="space-y-2">
              <input
                ...
                className="w-full border border-slate-200 dark:border-white/10 dark:bg-slate-800 dark:text-white rounded-lg px-3 py-2 text-xs"
              />
              <textarea
                value={message} onChange={(e) => setMessage(e.target.value)}
                placeholder={t("optionalMessage")} rows={2}
                className="w-full border border-slate-200 dark:border-white/10 dark:bg-slate-800 dark:text-white rounded-lg px-3 py-2 text-xs resize-none"
              />
              <button
                onClick={handleSendEmail}
                disabled={sending}
                className="w-full flex items-center justify-center gap-2 text-sm font-medium bg-violet-600 hover:bg-violet-700 text-white px-3 py-2 rounded-lg disabled:opacity-40 transition"
              >
